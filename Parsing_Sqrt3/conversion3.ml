...
let body_to_string (lss : expr list list) (param_func : ident) (spec_params : ident list): string = 
  let p1 = get_ls_from_lss lss  0 in let p3 = get_ls_from_lss lss 2 in 
  (*Printf.printf "valeur de la list let =\n %s \n "  (Eval.string_of_function_body (find_expr_let p3));*)
  let body1 = convert_val_of_expr p1 requires_body param_func spec_params p3 in 
  body1 ^ "=\n " ^  ls_expr (find_expr_let p3)
...
let conv ast name : string = 
  ...
  let spec_body = convert_spec_of_expr (get_ls_from_lss ast_body (index_spec+1)) requires_val var_let (get_decl_let_in_list (get_ls_from_lss ast_body 2))in 
