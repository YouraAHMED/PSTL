...
%token AXIOM INTO EQUIVALENT WHILE INVARIANT VARIANT AND DO DONE COLON_EQUAL IF THEN ELSE SEMI READS DIV
...
expr :    expr EMPTY expr {ASTBinOp ("<>", $1, $3)}
        ...
        | INTO {Into}
        | REF expr {Ref $2}
        | LPAR DIV LPAR expr RPAR expr RPAR {Div ($4,$6)}
        ...
        | READS LBRACE readlist RBRACE {Reads $3}
        | IDENT {ASTId($1)}
        | NUM {ASTNum($1)}
