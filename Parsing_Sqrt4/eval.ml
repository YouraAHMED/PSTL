...
let rec string_of_expr = function
  ...
  | Reads e -> "Reads (" ^  (unconcat_list (List.map string_of_expr e)) ^ ")"
  ...
  | While (e1, e2) -> "While (" ^ string_of_expr e1 ^ ", " ^ " do " ^ "[" ^ String.concat "; " (List.map string_of_expr e2) ^ "]" ^")"
